            ...
            <div class="card-body py-6">
                <form method="POST" action="{{ route('admin.masterdata.items.update', $item->id) }}" class="form">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" name="sku" value="{{ old('sku', $item->sku) }}" class="form-control @error('sku') is-invalid @enderror form-control-solid" placeholder="SKU item" required />
                        @error('sku')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-10">
                        <label class="form-label required">Nama</label>
                        <input type="text" name="name" value="{{ old('name', $item->name) }}" class="form-control @error('name') is-invalid @enderror form-control-solid" placeholder="Nama item" required />
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-10">
                        <label class="form-label required">CNT</label>
                        <input type="text" name="cnt" value="{{ old('cnt', $item->cnt) }}" class="form-control @error('cnt') is-invalid @enderror form-control-solid" placeholder="Informasi CNT" required />
                    ...
                    <div class="mb-10">
                        ...
                        <textarea name="description" class="form-control @error('description') is-invalid @enderror form-control-solid" rows="3" placeholder="Deskripsi item (opsional)">{{ old('description', $item->description) }}</textarea>
