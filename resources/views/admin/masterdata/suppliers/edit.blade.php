            ...
            <div class="card-body py-6">
                <form method="POST" action="{{ route('admin.masterdata.suppliers.update', $supplier->id) }}" class="form">
                    ...
                    <div class="mb-10">
                        ...
                        <input type="text" name="name" value="{{ old('name', $supplier->name) }}" class="form-control @error('name') is-invalid @enderror form-control-solid" placeholder="Nama supplier" required />
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Email</label>
                        <input type="email" name="email" value="{{ old('email', $supplier->email) }}" class="form-control @error('email') is-invalid @enderror form-control-solid" placeholder="Email supplier (opsional)" />
                        @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Telepon</label>
                        <input type="text" name="phone" value="{{ old('phone', $supplier->phone) }}" class="form-control @error('phone') is-invalid @enderror form-control-solid" placeholder="Telepon (opsional)" />
                        @error('phone')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Alamat</label>
                        <textarea name="address" class="form-control @error('address') is-invalid @enderror form-control-solid" rows="3" placeholder="Alamat (opsional)">{{ old('address', $supplier->address) }}</textarea>
