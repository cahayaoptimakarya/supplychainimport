...
@if(Perm::can(auth()->user(), 'admin.masterdata.uom.index', 'create'))
...
<script>
    ...
    const canUpdate = {{ \App\Support\Permission::can(auth()->user(), 'admin.masterdata.uom.index', 'update') ? 'true' : 'false' }};
    const canDelete = {{ \App\Support\Permission::can(auth()->user(), 'admin.masterdata.uom.index', 'delete') ? 'true' : 'false' }};
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const table = $('#uom_table').DataTable({
            ...
            ajax: {
                ...
                error: function(xhr){
                    ...
                    AppSwal.error('Gagal memuat data UOM');
                }
            },
            columns: [
                { data: 'id', name: 'id' },
        ...
        $('#uom_table').on('click', '.btn-delete', async function(e) {
            e.preventDefault();
            const url = this.getAttribute('data-url');
            const confirmed = await AppSwal.confirm('Yakin ingin menghapus UOM ini?', {
            ...
            fetch(url, {
                ...
                if (res.ok) {
                    ...
                    AppSwal.error('Gagal menghapus UOM');
                }
            }).catch(() => AppSwal.error('Gagal menghapus UOM'));
        });
